...
jsPsych.plugins["zeyirdkanimation"] = (function() {

	var plugin = {};

	plugin.info = {
	    name: "zeyirdkanimation",
	    parameters: {
		    // correct_choice: {
    ...
    const vh = Math.max(document.documentElement.clientHeight, window.innerHeight);
		...
		trial.aperture_center_y = vh*0.4;
    ...
    var islasttrial = 0;
		...
		*/
		...
		//Initialize stopping condition for tMotion function that runs in a loop
		...
		var firstFrame = true; //Used to skip the first frame in animate function below (in tMotion function)
		...
		//Declare a global timeout ID to be initialized below in tMotion function and to be used in after_response function
		var timeoutID;
		    ...
		    if(fixationCross === true){
	      	...
	      	if(border === true){
	        	...
	        	if(apertureType === 3 || apertureType === 4){
					...
					ctx.fillStyle = "#a8ee90";
					ctx.fillRect(0,xpos - vh * 0.1, vw, vh * 0.2);

					if (islasttrial == 1){
							ctx.fillStyle = "#000000"
							ctx.fillRect(bL-2*dotsize,xpos - vh * 0.1,2*dotsize,vh * 0.2);
							ctx.fillStyle = "#FF0000"
							ctx.fillRect(bR,xpos - vh * 0.1,2*dotsize,vh * 0.2);
					}
					if (islasttrial == -1){
							ctx.fillStyle = "#FF0000"
							ctx.fillRect(bL-2*dotsize,xpos - vh * 0.1,2*dotsize,vh * 0.2);
					...
					for (var i = 0; i<lines.length; i++){
							ctx.fillText(lines[i],  vw/2, vh* 0.1 + (i*lineheight) );
						}


							ctx.beginPath();

							ctx.arc(ypos, xpos, dotsize/2, 0, 2 * Math.PI, false);
							ctx.fillStyle = 'red';
							ctx.fill();
		...
		function updateDots() {
			...
			if ((islasttrial == 0) && (isstopmoving == 0)){
			...
			function animate() {
				...
				//Else continue with another frame request
					...
					if ( (!timerHasStarted) && (trial.trial_duration > 0) ){
						...
						timeoutID = window.setTimeout(function(){isstopmoving = 1;},trial.trial_duration); //This timeoutID is then used to cancel the timeout should the subject press a valid key
